...
class FlowExcel
{
	...
	public function openFlow()
	{
		...
		// Создание и запись новых файлов
                ...
                $str .= '<sst xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" count="' . ($this->options['coln'] * $this->options['rown']) . '" uniqueCount="' . ($this->options['coln'] * $this->options['rown']) . '">';
		file_put_contents(__DIR__ . $this->file_string, $str);*/

                touch(__DIR__ . $this->file_string);
                chmod(__DIR__ . $this->file_string, 0777);
                $f_string = fopen( __DIR__ . $this->file_string, 'a');
		fwrite($f_string, '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>' . "\r\n") or die('ERROR-ACCESS 3');
